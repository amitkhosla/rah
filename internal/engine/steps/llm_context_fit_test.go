...
// ── Plain text exceeds limit ─────────────────────────────────────────────────

func TestCheckContextFit_PlainText_Exceeds(t *testing.T) {
	ctx := newFitContext()
	// Build a long string — easily > 10 tokens worth
	ctx.ByteSlots[0] = []byte(strings.Repeat("word ", 200)) // 200 * 5 chars = 1000 chars ≈ 250 tokens
...
// ── MaxContextTokens=0 means no limit ────────────────────────────────────────
...
// ── JSON messages array in PromptSlot ────────────────────────────────────────
...
// ── Slot out-of-range handled gracefully (no panic) ───────────────────────────
...
// ── AlwaysContinues: step never returns StopPlan ─────────────────────────────
