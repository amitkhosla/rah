...
func TestDiagnosticURLExtraction(t *testing.T) {
	...
	if urls == nil || len(urls) == 0 {
		t.Log("[WARN] No URLs extracted (may be due to nested structure issues)")
		...
		for key, urlVal := range allURLs {
			...
			if len(issues) == 0 {
				t.Logf("[OK] %s → %s", key, urlVal)
			} else {
				t.Logf("[FAIL] %s → %s", key, urlVal)
	...
	for serviceKey, urlVal := range urls {
		...
		if len(issues) == 0 {
			validCount++
			t.Logf("[OK] %s → %s", serviceKey, urlStr)
		} else {
			invalidCount++
			...
			t.Logf("[FAIL] %s → %s", serviceKey, urlStr)
			for _, issue := range issues {
				t.Logf("   |-- %s", issue)
			}
		}
	...
	t.Log(strings.Repeat("=", 80))
	t.Logf("[OK] Valid URLs: %d", validCount)
	t.Logf("[FAIL] Invalid URLs: %d", invalidCount)
	t.Logf("Total: %d\n", len(urls))
...
func TestURLFilteringAndSanitization(t *testing.T) {
	...
	if len(urls) == len(validURLsOnly) {
		t.Log("\n[OK] All URLs are valid - no filtering needed")
	}
...
func TestEdgeCaseURLs(t *testing.T) {
	...
	for _, tc := range edgeCases {
		...
		if isValid == tc.valid {
			passed++
			if tc.valid {
				t.Logf("[OK] %s → VALID", tc.name)
			} else {
				t.Logf("[OK] %s → INVALID (detected: %v)", tc.name, issues)
			}
		} else {
			failed++
			t.Errorf("[FAIL] %s → Expected: %v, Got: %v (issues: %v)", tc.name, tc.valid, isValid, issues)
		}
	}
...
func TestURLFormatConsistency(t *testing.T) {
	...
	if len(issues) == 0 {
		t.Log("\n[OK] All URLs follow consistent format")
	} else {
		t.Log("\n[FAIL] Format inconsistencies found:")
		for _, issue := range issues {
			t.Logf("  - %s", issue)
		}
	}
	...
	for _, product := range config.Products {
		for _, category := range product.ServiceCategories {
			for _, svc := range category.Services {
				...
				if matched {
					t.Logf("[OK] %s → %s (matches schema)", key, url)
					validCount++
				} else {
