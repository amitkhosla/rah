...
//   - Runtime: matching value → then-branch (200)
//   - Runtime: non-matching value → else-branch (403)
//   - Regex flags: case-insensitive (i flag)
//   - Multiple flags combined (im)
//   - Invalid regex → compile-time error
//   - Unsupported flag → compile-time error
//   - Missing 'source' → compile-time error
//   - Missing 'pattern' → compile-time error
//   - Empty source slot (nil bytes) → no-match path
...
func TestPatternMatchEmptySlot(t *testing.T) {
	...
	// No header sent → slot is empty bytes → no-match
	ctx := compileAndRun(t, flows, []ApiUpdate{
		{Name: "pmApi", Path: "/pm", FlowName: "mainFlow", Action: "upsert"},
...
// ─── Test 6: compile-time error cases ────────────────────────────────────────
...
// ─── Test 7: instruction count verification ───────────────────────────────────
...
// ─── Test 8: dotall flag matches newline ──────────────────────────────────────
...
// ─── Test 9: step descriptor is registered ───────────────────────────────────
...
// ─── Test 10: isControlFlowAction includes pattern_match ─────────────────────
